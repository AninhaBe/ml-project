...
> Documento consolidado com a estrutura física do projeto e todos os arquivos necessários para recriar o agente no PC.

> Valores sensíveis devem ser preenchidos localmente no `.env`. Não coloque credenciais reais em repositórios.

---

## 🎯 O que esse agente faz

Lê catálogos de fornecedores via PDF ou entrada manual, identifica produtos no catálogo do Mercado Livre, analisa concorrência, calcula margem de contribuição e ranqueia oportunidades.

**Stack:** Python 3.10+, Streamlit, API Mercado Livre OAuth, Claude Vision opcional, SQLite, pandas/openpyxl.
...
Orquestra: cache → match → análise → margem
        ↓        ↓        ↓         ↓
     cache    matcher  analyzer   exporter
     SQLite   3 camadas concorr.   Excel
        ↓        ↓
  vision_llm   ml_api
  Claude       API Mercado Livre
```
...
- Python 3.10+
- Conta no DevCenter do Mercado Livre
- App criada no Mercado Livre com `client_id`, `client_secret` e `redirect_uri`
- API key da Anthropic, caso queira usar Claude Vision
...
## 🏗️ Passo 2 — Criar ambiente virtual
...
O `ML_REFRESH_TOKEN` será gerado pelo script `scripts/oauth_bootstrap.py`.

---

## 🔐 Passo 4 — Gerar token Mercado Livre

```bash
python scripts/oauth_bootstrap.py
```

Depois cole o `refresh_token` gerado no `.env`.
...
- 🟢 **APROVADO**: margem acima do mínimo e sem alertas críticos.
- 🟡 **AVALIAR**: margem aceitável, mas com alguma bandeira.
- 🔴 **REJEITAR**: margem insuficiente.
- 🔵 **REVIEW**: match incerto, precisa de validação humana.
- ⚪ **DESCARTADO**: sem catálogo válido ou sem match.
...
- `oportunidade_full`
- `mercado_full_only`
- `catalogo_sem_anuncios`

---

## 🔌 Endpoints Mercado Livre usados
...
# Preço "artístico" duplicado nos catálogos visuais: "3355" = 35, "RR$$" = R$
...
def parse_pdf_catalogo_visual(path: Path) -> list[ProdutoFornecedor]:
...
def parse_pdf_auto(path: Path) -> list[ProdutoFornecedor]:
    """Tenta ambos os parsers e retorna o que extrair mais produtos.

    1. Tenta parse_pdf_tabela (PDFs com tabelas estruturadas)
    2. Tenta parse_pdf_catalogo_visual (encartes / catálogos visuais)
    3. Retorna o que teve mais resultados
    """
    tabela = parse_pdf_tabela(path)
    visual = parse_pdf_catalogo_visual(path)
    logger.info(
        "auto parse: tabela=%d produtos, visual=%d produtos → usando %s",
        len(tabela), len(visual), "visual" if len(visual) > len(tabela) else "tabela",
    )
    return visual if len(visual) > len(tabela) else tabela
...
with tab_pdf:
    ...
    if pdf_file and st.button("📤 Processar PDF", type="primary"):
        ...
        with st.spinner("Extraindo produtos e imagens do PDF..."):
            produtos = parse_pdf_auto(pdf_path)

            # Extrai imagens do PDF e associa com produtos por página
            imgs_dir = IMGS_DIR / pdf_path.stem
            try:
                imagens = extrair_imagens_pdf(pdf_path, imgs_dir)
                # Agrupa imagens por página (page000_img000.png)
                imgs_por_pagina: dict[int, list[Path]] = {}
                for img_path in imagens:
                    # Extrai número da página do nome: page003_img001.png → 3
                    import re as _re
                    m = _re.search(r"page(\d+)", img_path.name)
                    if m:
                        pg = int(m.group(1))
                        imgs_por_pagina.setdefault(pg, []).append(img_path)

                # Associa a primeira imagem da página ao produto
                associados = 0
                for p in produtos:
                    imgs = imgs_por_pagina.get(p.pagina, [])
                    if imgs and not p.imagem_path:
                        p.imagem_path = imgs[0]  # primeira imagem da página
                        associados += 1

                st.info(
                    f"Extraídos {len(produtos)} produtos e {len(imagens)} imagens "
                    f"({associados} produtos com foto para Vision AI)."
                )
            except Exception as e:
                logging.warning("Erro extraindo imagens: %s", e)
                st.info(f"Extraídos {len(produtos)} produtos do PDF (sem imagens).")
...
```env
TAXA_ML_CLASSICO=0.12
TAXA_ML_PREMIUM=0.17
```
...
## Ajustar thresholds de match

Em `src/matcher.py`, altere:
...
O `code` pode ter expirado ou o `redirect_uri` pode estar diferente do cadastrado no DevCenter.

## HTTP 403

Pode ser endpoint sem permissão para terceiros ou token inválido.

## HTTP 429

Rate limit. Aguarde e rode novamente.

## Claude Vision desativado
...
## Parser de PDF retornou 0 produtos

O parser atual funciona melhor em PDFs com texto/tabela. Encartes em imagem podem exigir processamento manual ou parser visual.
...
- Review humano lado a lado
- Parser visual para encartes
- Histórico de preços
- Detecção de catálogos duplicados
...
- Estimativa de vendidos por página pública

## Longo prazo

- Integração com dados do Avant/Shopping de Preços
- Modelo para Buy Box
- Multi-marketplace
...
- [ ] App criada no DevCenter do Mercado Livre
- [ ] `.env` preenchido
- [ ] OAuth bootstrap executado
- [ ] `ML_REFRESH_TOKEN` salvo no `.env`
- [ ] `streamlit run app.py` funcionando
- [ ] Primeira análise manual executada
- [ ] Exportação Excel testada

---

*Documento final consolidado em 2026-06-22 04:50.*
